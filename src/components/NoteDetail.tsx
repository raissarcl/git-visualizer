...
interface NoteDetailProps {
  ...
  checkBranch: (repo: string, branch: string) => Promise<'verified' | 'missing'>
...
export function NoteDetail({
  ...
  const dirty = Boolean(
    ...
    draft &&
    note.id === draft.id &&
    !workspaceNotesContentEqual(note, draft),
  )
  ...
  const handleCheck = async () => {
    if (draft.link.type !== 'branch' || !draft.link.branch.trim() || !token)
      return
    setChecking(true)
    setCheckError(null)
    try {
      ...
      setCheckError(
        err instanceof Error ? err.message : 'Falha ao verificar branch.',
      )
    } finally {
      setChecking(false)
    }
  ...
  return (
    <>
      <DetailDrawer aria-label="Detalhe da nota" onClose={handleClose}>
        ...
        <label className="note-title-field">
          ...
          <input
            ...
            onChange={(e) =>
              setDraft((d) =>
                d ? patchDraft(d, { title: e.target.value }) : d,
              )
            }
          />
        </label>
        ...
        <div className="note-link-block">
          ...
          {draft.link.type === 'branch' && (
            <>
              ...
              {checkError && (
                <p className="filters-backup-error">{checkError}</p>
              )}
        ...
        {draft.linkedPr && (
          <div className="note-pr-linked">
            <span>
              Vinculada ao PR #{draft.linkedPr.number} ·{' '}
              <code>{draft.linkedPr.repo}</code>
            </span>
            <button
              type="button"
              className="btn"
        ...
        <div className="detail-notes">
          ...
          <p className="detail-notes-hint">
            Markdown · só neste navegador · use Salvar
          </p>
          {bodyMode === 'edit' ? (
            <textarea
              ...
              onChange={(e) =>
                setDraft((d) =>
                  d ? patchDraft(d, { body: e.target.value }) : d,
                )
              }
        ...
        <div className="detail-actions">
          ...
          <button
            type="button"
            className="btn btn-backup-danger"
      ...
      <ConfirmActionModal
        ...
        confirmLabel={
          confirmKind === 'delete' ? 'Excluir nota' : 'Salvar e fechar'
        }
        cancelLabel={
          confirmKind === 'delete' ? 'Manter nota' : 'Continuar editando'
        }
        secondaryLabel={
          confirmKind === 'save-close' ? 'Fechar sem salvar' : undefined
        }
